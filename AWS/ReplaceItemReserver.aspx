<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="ReplaceItemReserver.aspx.cs" Inherits="ReplaceItemReserver" MaintainScrollPositionOnPostback="true" %>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            $.postJson('GetValueByCode.ashx', { mode: "replaceitem", center_code: _center_code }, function(d, s) {
                if (s == "success") {
                    //$('#ctl00_ContentPlaceHolder1_TabContainer1_TabPanel_cneterSel').next().html(d["status"]);
...
<ajaxToolkit:TabPanel runat="server" ID="TabPanel" HeaderText="替代項目報進作業">

<ContentTemplate>
<div id="BusyBoxDiv" style="display:none;">
	<span style="color: #ff8000; width: 200px; font-size: 30px; font-weight: bold; text-align: center;"><img src="images/processing.gif"/> 替代項目報進作業中...</span>
</div>
...
<div id="OrderStep1" runat="server">
                ...
                <tr>
                    <td class="Order_Section_tr3_td1">替代項目列表</td><td class="Order_Section_tr3_td2">
...
<div id="zerorder" style="display:none;" runat="server">
    ...
    OnRowDataBound="GridView2_OnRowDataBound" OnDataBound="GridView2_OnDataBound">
        <Columns>
            ...
            <asp:HyperLinkField Text="變更" Target="_self" DataNavigateUrlFields="center_code,date" DataNavigateUrlFormatString="ReplaceItemReserverChange.aspx?center_code={0}&date={1}" HeaderText="變更"/>
            <asp:HyperLinkField Text="刪除" Target="_self" DataNavigateUrlFields="center_code,date" DataNavigateUrlFormatString="ReplaceItemReserverCancel.aspx?center_code={0}&date={1}" HeaderText="刪除"/>
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="SqlDataSource2" runat="server" 
        ConnectionString="<%$ ConnectionStrings:MainDB %>"
        SelectCommand="SELECT DISTINCT CONVERT(varchar(10), date,111) AS date, (SELECT center_name FROM Center AS c WHERE (center_code = r.center_code)) AS center_name, op_id, center_code FROM Result AS r WHERE (r.id = @op_id and r.status = '000' and r.memo <> '000' and r.date > @date and (result is NULL or result = '777') )">
            <SelectParameters>
