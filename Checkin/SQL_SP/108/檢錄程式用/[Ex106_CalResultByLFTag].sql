...
--建立[Ex106_CalResultByLFTag]--
--重新建立一隻新的sp，供106年1月1日後使用晶片結算成績計算--
CREATE procedure [dbo].[Ex106_CalResultByLFTag]
(
@date datetime,
@lf_tag varchar(10)
)
...
begin
   ...
   begin
     ...
     into #triple from StandardEncrypt_106 where gender = @gender and @age <= agemax and @age >= agemin
     ...
     CONVERT(smallint,  DECRYPTBYKEY([standard])) 'standard',
     CONVERT(smallint,  DECRYPTBYKEY([score])) 'score'
     into #repl from ReplaceStandardEncrypt_106 where gender = @gender and @age <= agemax and @age >= agemin
